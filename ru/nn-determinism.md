...
В этом сулчае следует признать проблему, указать на ее наличие.
...
По результатам анализа проблем должна быть безусловно указана и устраненна.
...
включена в аргументы модели, с объяснением цели и источника, а так же с 
возможностью ее последовательного испытания на идентичных аргументах. При этом 
модель обязана по прежнему демонстрировать детерминируемость.
...
контроля.
4. # Принципиальная детерминированность нейросетей и безопасность
...
должно основываться на понимании источников недетерминированности её описания и 
контроля.
4. Реализация реализация детерминированного пайплайна исполнения нейросети 
должно быть приоритетной сознательной целью.


# Заключение

Безопасность развития нейросетей должно быть основано не на обсуждении 
этических вопросов, законов работотехники, социальных проблем, а в первую очередь
на достижении гарантироемой детерминируемости и проверяемости результатов. 

---

PS. Недетерминизм - это баг а не фича в вашем пайплайне, найдите его!
