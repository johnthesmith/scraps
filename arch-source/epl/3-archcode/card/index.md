Карточка любой [](entity) генерируется из шаблона, указанного в самой сущности и 
ее свойств. 
...
1. Шаблон формируется из [](md) документа, который может включать подмены в
следующем формате:
...
```
    1. где:
        ...
        path - опциональный путь к своству [](entity)
